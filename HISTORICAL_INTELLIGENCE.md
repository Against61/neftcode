...
Первые два вида контекста доступны сразу из пакета `models/`.
Исторические аналоги требуют локального индекса: он содержит строки и
времена обучения и поэтому не публикуется. В этом случае ответ содержит
`analogs=[]` и `analogs_status=NOT_BUNDLED_REBUILD_FROM_LOCAL_HISTORY`.
...
Для готового prediction-only пакета достаточно:
...
Локальная конфигурация и SHA модели подключаются автоматически. Для
своего full bundle с индексом аналогов остаётся явный режим:
...
как 5/15 вместо требуемых 9/15. В full bundle пять аналогов доступны как
диагностическое обоснование без причинных выводов.
